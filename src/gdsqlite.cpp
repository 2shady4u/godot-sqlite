...
bool SQLite::import_from_json(String import_path)
{
    ...
    for (auto table : tables_to_import)
    {
        ...
        for (int i = 0; i <= table.base64_columns.size() - 1; i++)
        {
            String key = table.base64_columns[i];
            for (int j = 0; j <= table.row_array.size() - 1; j++)
            {
                Dictionary row = table.row_array[j];
    ...
    for (int i = 0; i <= number_of_tables - 1; i++)
    {
        ...
        if (!query_result.empty())
        {
            ...
            if (!base64_columns.empty()){
                table_dict["base64_columns"] = base64_columns;
            }
        }
...
bool SQLite::validate_json(Array import_json, std::vector<table_struct> &tables_to_import)
{
    ...
    for (int i = 0; i <= number_of_tables - 1; i++)
    {
        ...
        new_table.base64_columns = get_with_default(temp_dict, "base64_columns", Array());
